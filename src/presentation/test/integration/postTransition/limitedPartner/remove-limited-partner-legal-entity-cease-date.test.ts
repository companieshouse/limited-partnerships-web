...
describe("Limited Partner LegalEntity cease date page", () => {
  ...
  describe("GET limited partner legal entity cease date page", () => {
    ...
    ])("should load limited partner legal entity cease date page with %s text", async (lang: string, translationText: any) => {
      const res = await request(app).get(URL + `?lang=${lang}`);

      expect(res.status).toBe(200);
      expect(res.text).toContain(`${translationText.partner.ceaseDate.removeLimitedPartner.title}`);
