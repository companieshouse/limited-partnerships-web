...
jest.mock("../../controller/global/Controller", () => {
  return function () {
    return {
      ...
      getConfirmationPagePostTransition: jest.fn().mockImplementation(() => {
        return (_req: Request, _res: Response, next: NextFunction) => {
          return next();
        };
      }),
      resumeJourneyRegistrationOrTransition: jest.fn().mockImplementation(() => {
        return (_req: Request, _res: Response, next: NextFunction) => {
          return next();
        };
      }),
      resumeJourneyPostTransition: jest.fn().mockImplementation(() => {
        return (_req: Request, _res: Response, next: NextFunction) => {
          return next();
        };
      })
