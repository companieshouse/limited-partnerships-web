...
describe("PSC usual residential Address Territory Choice", () => {
  ...
  describe("GET PSC usual residential address territory choice", () => {
    it.each([
      ["English", "en", enTranslationText],
      ["Welsh", "cy", cyTranslationText]
    ])(
      "should load the PSC usual residential address territory choice page with %s text",
      async (_description: string, lang: string, translationText: Record<string, any>) => {
        ...
        expect(res.text).toContain(
          toEscapedHtml(
            `${translationText.address.territoryChoice.personWithSignificantControlUsualResidentialAddress.title} - ${translationText.serviceRegistration} - GOV.UK`
          )
        );

        testTranslations(
          res.text,
          translationText.address.territoryChoice.personWithSignificantControlUsualResidentialAddress
        );
  ...
  describe("POST PSC territory choice", () => {
    it.each([
      ...
      [
        "Overseas",
        "overseas",
        getUrl(ENTER_PERSON_WITH_SIGNIFICANT_CONTROL_INDIVIDUAL_PERSON_USUAL_RESIDENTIAL_ADDRESS_URL)
      ]
    ])(
