...
describe("Choose usual residential address individual person page", () => {
  const enTranslationText = { ...enGeneralTranslationText, ...enAddressTranslationText };
  const cyTranslationText = { ...cyGeneralTranslationText, ...cyAddressTranslationText };
  const URL = getUrl(CHOOSE_PERSON_WITH_SIGNIFICANT_CONTROL_INDIVIDUAL_PERSON_USUAL_RESIDENTIAL_ADDRESS_URL);
  ...
  describe("GET choose usual residential address individual person", () => {
    it.each([
      ["English", "en", enTranslationText],
      ["Welsh", "cy", cyTranslationText],
    ])(
      "should load the choose usual residential address individual person page with %s text",
      async (_description: string, lang: string, translationText: Record<string, any>) => {
        ...
        testTranslations(
          res.text,
          translationText.address.chooseAddress.personWithSignificantControlUsualResidentialAddress
        );
  ...
  describe("POST choose usual residential address individual person page", () => {
    it("should redirect to the next page and add select address to cache for %s", async () => {
      const res = await request(app)
        .post(URL)
        .send({
          pageType: AddressPageType.choosePersonWithSignificantControlIndividualPersonUsualResidentialAddress,
          selected_address: `{
              "postal_code": "ST6 3LJ",
              "premises": "4",
