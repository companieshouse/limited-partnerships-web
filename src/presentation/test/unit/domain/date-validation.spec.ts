import {
  dateContainsInvalidChars,
  isDateInPast,
  isDateToday,
  isValidDate,
  isValidDateStringAndNotInFuture,
  validateDateOfBirth,
  DateErrorMessages,
  isDateInPastOrToday,
  isBeforeRegistrationDate
} from "../../../../domain/validator/DateValidators";
...
describe("isDateInPastOrToday", () => {
  ...
  it("returns true for a date before today", () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date(2026, 4, 21, 10, 0, 0));
    expect(isDateInPastOrToday("20", "05", "2026")).toBe(true);
  });
  ...
  it("returns true for today", () => {
    ...
    expect(isDateInPastOrToday("21", "05", "2026")).toBe(true);
  });

  it("returns false for a future date", () => {
    jest.useFakeTimers();
    jest.setSystemTime(new Date(2026, 4, 21, 10, 0, 0));
    expect(isDateInPastOrToday("22", "05", "2026")).toBe(false);
  });

  it("returns false for invalid date values", () => {
    expect(isDateInPastOrToday("32", "13", "abcd")).toBe(false);
  });
...
describe("isBeforeRegistrationDate", () => {
  it("returns true if the date is before the registration date", () => {
    expect(isBeforeRegistrationDate("20", "05", "2023", "2023-12-29T11:47:59.193Z")).toBe(true);
  });

  it("returns false if the date is the same as the registration date", () => {
    expect(isBeforeRegistrationDate("21", "05", "2026", "2026-05-21T10:00:00.000Z")).toBe(false);
  });

  it("returns false if the date is after the registration date", () => {
    expect(isBeforeRegistrationDate("22", "05", "2026", "2026-05-21T10:00:00.000Z")).toBe(false);
  });
});
...
describe("validateDateOfBirth", () => {
  ...
  const testErrorMessages: DateErrorMessages = {
    ...
    invalidChars: "Date contains invalid characters",
    invalid: "Date is invalid",
    notInPast: "Date of birth is not in the past"
  };
  ...
  describe("missing date fields", () => {
    it("sets error when all fields are missing", () => {
      ...
      expect(mockUIErrors.setWebError).toHaveBeenCalledWith(DATE_OF_BIRTH_FIELD, testErrorMessages.missing);
    });
  ...
  describe("invalid characters", () => {
    it("sets error when day contains non-numeric characters", () => {
      ...
      expect(mockUIErrors.setWebError).toHaveBeenCalledWith(DATE_OF_BIRTH_FIELD, testErrorMessages.invalidChars);
    });
  ...
  describe("invalid dates", () => {
    it("sets error for leap day in a non-leap year", () => {
      ...
      expect(mockUIErrors.setWebError).toHaveBeenCalledWith(DATE_OF_BIRTH_FIELD, testErrorMessages.invalid);
    });
  ...
  describe("date not in past", () => {
    ...
    it("sets error for a future date", () => {
      ...
      expect(mockUIErrors.setWebError).toHaveBeenCalledWith(DATE_OF_BIRTH_FIELD, testErrorMessages.notInPast);
    });
